...
def test_catalogue_cron_matches_pg_cron(migration_jobs):
    ...
    mismatches = [
        ...
        for name, job in migration_jobs.items()
        if name in by_job and by_job[name].cron != job["cron"]
    ]
    assert not mismatches, "\n".join(mismatches)
...
def test_only_article_curate_is_enabled_today():
    assert [e.kind for e in wc.enabled_entries()] == ["article_curate"]

# goboard_final.py
# python goboard_final.py                → pure ui (instant)
# python goboard_final.py --tda --swarm  → full sentient mode

import argparse, random, platform
parser = argparse.ArgumentParser()
parser.add_argument("--tda", action="store_true")
parser.add_argument("--swarm", action="store_true")
parser.add_argument("--domain", type=str, default="pension asset allocation")
...
if args.tda or args.swarm:
    ...
    if args.tda:
        try:
            ...
            print("giotto-tda missing → pip install giotto-tda")
    if args.swarm:
        try:
...
class DomainModal(ModalScreen):
    def compose(self):
        ...
        yield Input(id="domain-in", placeholder="pension asset allocation")
    def on_input_submitted(self, e):
        if e.value.strip() and SWARM:
            self.app.domain = e.value.strip()
            self.app.query_one("#status").update(f"Domain → {self.app.domain}")
        self.dismiss()
...
class GoBoardApp(App):
    ...
    .yellow { background: yellow; color: black; text-style: bold; }
    .panel { background: #111; border: tall #555; padding: 1; margin: 1; }
    ...
    def compose(self) -> ComposeResult:
        ...
        if SWARM: menu += "[s] SWARM [d] DOMAIN "
        ...
        yield Static("Ready | hjkl=move o=overlay", id="status")
    ...
    # Optional swarm actions
    if SWARM:
        def action_show_domain(self): self.push_screen(DomainModal())
        @work
        async def action_run_swarm(self):
            log = self.query_one("#log", ListView)
            log.clear()
            log.append(ListItem(Static(f"[bold green]Swarm active in domain: {self.domain}[/]")))
            emit_reward("demo", 1.0)

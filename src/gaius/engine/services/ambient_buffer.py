...
class AmbientBuffer:
    ...
    async def compact_if_needed(self, summarize) -> dict[str, object]:
        ...
        if not (summary or "").strip():
            raise RuntimeError(f"{GURU}\n  empty summary")
        # A summary larger than the material it replaces is not a summary. On
        # 2026-09-06 01:10 a 159 946-byte reasoning trace was accepted here as
        # the SUMMARY of a 79 KB window; every compaction after it re-read the
        # dump, reasoned to the token cap and failed, and the ambient class was
        # dark for 7 h. The first guard (summary ≤ max_bytes/4) then broke the
        # FMP roll (09:45–15:38): its compaction legitimately rolls a 110–130 KB
        # market state forward out of ~170 KB of prior summary + content. So the
        # criterion is RELATIVE to what is dropped (the trace was 2× its material)
        # with an absolute ceiling of three quarters of the buffer.
        summary_bytes = len(summary.strip().encode("utf-8"))
        dropped_bytes = sum(int(e.content_bytes) for e in snapshot[: plan.cut_index])
        ceiling = int(self._max_bytes * 0.75)
        cap = max(min(dropped_bytes, ceiling), 4096)
        if summary_bytes > cap:
            raise RuntimeError(
                f"#BUF.00000002.SUMMARYBLOAT summary is {summary_bytes} bytes for "
                f"{dropped_bytes} bytes of dropped material (ceiling {ceiling} = 3/4 of "
                f"max_bytes {self._max_bytes}): not a summary; nothing written, the "
                "window stays intact for the next attempt"
            )

 ...
 **A kappa-style streaming architecture enables parallel voice processing where a fast edge agent and deep reasoning agent collaborate through a unified call bridge—delivering both instant responsiveness and sophisticated understanding in real-time voice interactions.**
 ...
 ┌─────────────────────┐          │          ┌─────────────────────┐
 │    FAST PATH        │          │          │    DEEP PATH        │
 │  (Edge Consumer)    │          │          │  (Reasoning Agent)  │
 ├─────────────────────┤          │          ├─────────────────────┤
 │ • Whisper STT       │          │          │ • Grok Voice API    │
 │ • Intent Detection  │          │          │ • Complex Reasoning │
 │ • Widget Commands   │          │          │ • Web/X Search      │
 │ • 3D Navigation     │          │          │ • Tool Execution    │
 │ • Quick Acks        │          │          │ • Nuanced Response  │
 └──────────┬──────────┘          │          └──────────┬──────────┘
            │                     │                     │
            │            ┌────────┴────────┐            │
            │            │  TURN ARBITER   │            │
 ...
 **Configuration for Deep Agent**:
 ...
 ```
               ...
               ┌──────────────┴──────────────┐
               │                             │
     Simple command/ack             Complex query detected
               │                             │
               ▼                             ▼
     ┌─────────────────┐           ┌─────────────────┐
     │  FAST_SPEAKING  │           │  DEEP_THINKING  │
     │ (Edge responds) │           │ (Fast fills gap)│
     └────────┬────────┘           └────────┬────────┘
              │                             │
     Response complete             Deep agent ready
              │                             │
              │                             ▼
              │                   ┌─────────────────┐
              │                   │  DEEP_SPEAKING  │
 ...
 ┌────────────────────────────────────────┐
 │           AUDIO ARBITER                │
 │  • Receives from active agent only     │
 │  • Buffers during handoffs             │
 ...
 **Key principle**: Only the agent with floor control has its audio routed to the mixer. The arbiter maintains a small buffer (50-100ms) to enable smooth crossfades during handoffs.
 ...
 ```
 User                CallBridge(DO)        FastPath              DeepPath(Grok)
   │                      │                    │                       │
   │  "What's the geopolitical impact of the CHIPS Act on Taiwan?"     │
   │─────────────────────▶│                    │                       │
   │                      │ audio_event(1)     │                       │
   │                      │───────────────────▶│                       │
   │                      │                    │ [transcribe]          │
   ...
   │                      │ set_floor(deep)    │                       │
   │                      │                    │                       │
   │ "The CHIPS Act has created significant   │                       │
   │  pressure on Taiwan's semiconductor      │◀─────audio_stream─────│
   │  dominance. Three key impacts..."        │                       │
   │◀─────────────────────────────────────────────────────────────────│
   │                      │                    │                       │
   │                      │ response_complete  │                       │
   │                      │◀─────────────────────────────────────────────

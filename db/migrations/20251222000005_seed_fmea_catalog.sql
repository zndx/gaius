...
-- Infrastructure Failures (4 modes)
INSERT INTO fmea_catalog (failure_mode_id, category, name, description, base_severity, base_occurrence, base_detection, detection_method, recommended_actions, escalation_tier) VALUES
...
-- MetaAgent Failures (new category for analytics system)
INSERT INTO fmea_catalog (failure_mode_id, category, name, description, base_severity, base_occurrence, base_detection, detection_method, recommended_actions, escalation_tier) VALUES
('META_001', 'metaagent', 'Cypher Query Failure', 'AGE graph Cypher query returning errors', 5, 4, 3, 'health_check', ARRAY['retry_query', 'fallback_sql', 'check_graph'], 0),
('META_002', 'metaagent', 'Analyst Timeout', 'MetaAgent analyst taking >30s to respond', 4, 5, 4, 'metric_threshold', ARRAY['retry_analyst', 'reduce_scope', 'skip_analyst'], 0),
('META_003', 'metaagent', 'Correlator Empty', 'Correlator receiving no analyst findings', 6, 4, 5, 'health_check', ARRAY['retry_analysts', 'widen_scope', 'check_data'], 0),
('META_004', 'metaagent', 'Entity Resolution Failure', 'KB semantic search returning no matches', 3, 5, 4, 'health_check', ARRAY['broaden_query', 'use_literal', 'check_embeddings'], 0)
ON CONFLICT (failure_mode_id) DO UPDATE SET
    name = EXCLUDED.name,
    description = EXCLUDED.description,
    base_severity = EXCLUDED.base_severity,
...
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'GPU_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'VLLM_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'MQ_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'EV_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'EB_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'RC_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'INFRA_%';
DELETE FROM fmea_catalog WHERE failure_mode_id LIKE 'META_%';

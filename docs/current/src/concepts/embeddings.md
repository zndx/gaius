...
```
"pension fund"     → [0.23, -0.41, 0.88, ...]  (128 dims per token)
"retirement plan"  → [0.25, -0.39, 0.86, ...]  (nearby)
"pizza recipe"     → [-0.67, 0.12, -0.33, ...] (distant)
```

Gaius uses ColBERT-Zero (`lightonai/ColBERT-Zero` via pylate) for every embedding: 128-dim per-token vectors scored by MaxSim, plus a mean `agg` vector per document for projection. The engine loads it on demand on the light GPU (`gaius.engine.embeddings.colbert`); there is no vLLM embedding endpoint.

## Point Clouds in Gaius

When multiple embeddings are collected — knowledge base documents, agent utterances, card content — they form a **point cloud** in the 128-dimensional `agg` embedding space. This point cloud is the raw material for:

1. **Grid projection** — UMAP reduces the 128-dim `agg` vectors to 2D, then quantized to the 19x19 grid
...
```
128-dim ColBERT-Zero `agg` embeddings (Qdrant `gaius_kb_colbert_zero`)
    │
    ▼
...
UMAP preserves local neighborhood structure — points that are close in `agg` space remain close on the grid. The projection is necessarily lossy. This is a feature: it forces salience. Points that survive projection and remain distinct are points that matter.
...
These analyses operate on the original 128-dim `agg` embeddings, not the projected 2D coordinates. The grid shows *where* features appear; the topology describes *what* features exist.
...
- **Agent collaboration**: LatentMAS uses Qdrant as shared working memory (128-dim ColBERT-Zero `agg` dense or 20,480-dim CLT sparse)

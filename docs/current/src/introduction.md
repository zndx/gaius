...
| **Theta cycle** | Monday 06:00 UTC, Airflow DAG `gaius_theta_cycle`, `ThetaCycleFlow`: encode the **previous ISO week's** `cognition_thoughts` through `EmbedTexts`, NVAR drift, BERTSubs subsumption, knowledge-gradient selection, KB augmentation; objective `theta_cycle` ([Theta Consolidation](./architecture/theta.md)). |

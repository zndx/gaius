# Hierarchical soak tick (23:36Z)

Hour **496559** still hot (~36 min). Below analog threshold (≥55 min /
...
Jsonl sidecar `pid 36619` (~9.7 h, sample-only) still up. Engine
`pid 4115269` (started 21:16:19 this life) still up. Warehouse ingest
`ticks=8100 inserted=48600` by 23:35:55. Lag ~0.2–0.7 s. Live sample
`power_w=116–131` `util=100` `mem=21435` on GPUs 0/2/3, gpu-5 `mem=3`
(real DCGM). Jsonl 2170 ticks / 13020 rows vs Kudu **12780** rows
(2130 s) — short **~40** warehouse ticks from the 23:21 DROP catalog
stall, not padded.
...
- `SHOW RANGE` starts at `VALUE = 496559` … `496582`. UNION honest:
  ...
  | 496559 | Kudu    | 12780 live |

- Strip=1h UI+CLI `driver=warehouse`, `n_times=3600`, matrix **72000**,
  epoch 23:36:30Z. gpu-0..5 nonzero 3504/3600 (honest recycle gaps).

# Hierarchical soak tick (11:36Z)

Sampler still C++ (`pid 2989721`, ~7.4 h, `kudu_ok`). Hour **496547**
hot (~36 min jsonl). Impala FE already `IMPALA_BUILD_OK`. Do not analog
— not closed (12:00Z). Hour **496546** already Iceberg.

## Proven (no fake rows)

- Live C++ upsert of jsonl hour 496547: **12996** rows (2166 ticks).
  Sample `power_w=8.7` `util=0` `mem=21334` (real nvidia-smi; GPUs
  idle this tick).

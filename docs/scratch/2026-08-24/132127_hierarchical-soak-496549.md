# Hierarchical soak tick (13:21Z)

Sampler still C++ (`pid 2989721`, ~9.1 h, `kudu_ok`). Hour **496549**
hot (~21 min jsonl). Impala FE already `IMPALA_BUILD_OK`. Do not analog
— not closed (14:00Z). Hour **496548** already Iceberg.

## Proven (no fake rows)

- Live C++ upsert of jsonl hour 496549: **7548** rows (1258 ticks).
  Sample `power_w=107` `util=24` `mem=21334` (real nvidia-smi).
- FDW UNION `:5455` `gpu_metrics` `impala_sql`:
  ...
  lag ~5 s after upsert. Sampler kept writing after the C++ upsert,
  so FDW is ahead of the 7548 snapshot.

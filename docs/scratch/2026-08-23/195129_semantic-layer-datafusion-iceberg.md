...
| **Date** | 2026-08-23 |
| **Status** | Draft (Q4/Q5/Q6 decided 2026-08-23) |
...
HDF5 is treated honestly: `Hdf5FormatModel` is a **pointer-table / metadata-plane façade**, not a JVM Iceberg `FormatModel`. File Format API integration is graduation step 2. `hdf5_iceberg` stays a standalone package (path-dep / submodule / `packages/hdf5_iceberg`), **not** Gaius `thirdparty/`. **This program drives `hdf5_iceberg` to a complete shared implementation** (pointer tables, kerchunk, overviews, SysML/SDG audit, virtual-block grid) used by Gaius **and** cyberphy. Polarisfork append may stay in Gaius `hx` so the library remains catalog-agnostic.
...
3. **Worked path:** SysML v2 / SDG-grounded HDF5 → `hdf5_iceberg.register_root` → Gaius `hx` Polarisfork pointer tables → Dask hyperslab (script / cyberphy Navigator as consumer) **and** DataFusion SQL on Parquet overviews / **settled** Iceberg strip. Metabase in-line mapping when the SQL face exists.
...
6. **Keep `hdf5_iceberg` standalone** (path-dep/submodule/`packages/hdf5_iceberg`); do not put it in Gaius `thirdparty/`. **Do not defer virtual blocks, kerchunk, overviews, or SysML/SDG audit to cyberphy** — drive those into a **complete shared package**. Polarisfork append may stay in Gaius `hx`. cyberphy Navigator remains a consumer; Gaius may land missing library pieces.
...
| K8 | **`hdf5_iceberg` stays a standalone shared package — and this program completes it.** Consume via path-dep or submodule from `cldr/cybersec/packages/hdf5_iceberg`, or a `packages/hdf5_iceberg` copy with `scripts/vendor-hdf5-iceberg.sh` + NOTICE. **Not `thirdparty/`**. **In scope here:** pointer-table model, kerchunk refs, overview pyramids, SysML/SDG audit, virtual-block grid (K20), Dask read path. Gaius may land missing pieces in the origin package; they must not live as a Gaius-only overlay forever. **Polarisfork Iceberg append may stay in Gaius `src/gaius/hx/`** wrapping `register_root` + `get_catalog()` so the library stays catalog-agnostic. cyberphy Navigator is a **consumer**; it must not import Gaius. | Avoid circular deps. Do not defer the format/audit/Dask path to cyberphy. See A6. |
...
| K20 | **HDF5 virtual-block starting grid lives in `hdf5_iceberg` (Q6).** Series-block target **8 MiB** on the Values **int16** matrix: `series_block_rows = floor(8 MiB / (n_time × 2))`. Gaius analog `N=5001`, `T=10000` → **419 rows/block** (419 × 10000 × 2 = 8 380 000 B ≈ 7.99 MiB). Coalesce **8** blocks → **~64 MiB** Dask tasks (`task_target_bytes = 64 MiB`). Then **measure** on the Gaius analog and **record the measured grid in the library** (constants + a short note in `hdf5_iceberg`, not only a Gaius script). Design #42’s open “virtual block size” item is **closed here as the starting grid**; tune from that measurement. cyberphy Navigator consumes the same defaults. | Do not defer to cyberphy. Format/audit/Dask path is this program. |
...
Four merge queues (Gaius, Signals, Metabase, **hdf5_iceberg origin**). Artifact of Signals PRs is a **versioned `signals-df` binary** (Engine) **plus** `impala_fdw` `.so` and `sdg.warehouse` DDL (**Kudu + Iceberg + UNION**). Projector syncs the **existing** Metabase Postgres Database (`:5432/sdg`) — **no driver JAR**. Gaius PRs G1–G4 unit-test without `:8181`.
...
**Merge order for a full demo:** PR-H1 (library), PR-S1 (Engine DF Iceberg listing), PR-S2 (FDW Iceberg), PR-S3 (Kudu DDL + UNION FTs on `:5432`), PR-M1, Gaius G1–G8 with G8 cards last. **Gaius Engine v1 without Metabase cards** is H1 + G1–G7 + G8 YAML + G9 + S1 (DF listing will be empty of strip until first settle; overviews/pointer still work). **Metabase v1 requires S3 UNION + G6 Kudu writer.**

...
Gaius and Signals already store two very different scientific grains: (1) dense HDF5 acquisitions whose native layout is a 2-D `Values[series × time]` hyperslab (PRODML DAS-shaped; cyberphy's OTel analog is the structural stand-in), and (2) a 10 Hz named-channel strip (`gpu-0..5`, `util-0..5`, `kv`, `run`, `gen-tps`, `prefill`, `clt`, `sae`, `ricci`, `ricci-d`) that is a 200-scalar/s time series, not an HDF5 cube. Neither grain is a Metabase "Number" column, and neither is a drop-in Iceberg `FileFormat` today.

This design introduces a **Gaius-owned semantic catalog** (Iceberg tables in Polarisfork) that binds every measure to an SDG OWL / SysML v2 annotation: unit, quantity kind, grain, join keys, and **legal aggregations** (rows in `semantic.aggregations`, not a list column). **Warehouse SQL engine is Impala HS2 `:21050`.** Polarisfork is Iceberg REST catalog only. **`impala_fdw` is enhanced to reach both Kudu and Iceberg** (this initiative **lifts** SPEC G6/N2) and is installed on **three Postgres fronts**: (1) **Signals devenv Postgres `:5455`** (local lab, implement-now); (2) **system Postgres `:5432`** (Metabase); (3) **ElectricSQL PGlite** as an AMP/CAI **SQL-contract** target (Atelier `scripts/pglite-server.mjs` on **`:5440`**; Atelier local devenv PG is **`:5533`** — ports are not interchangeable). **Do not call Signals `:5455` “pglite”** (older Signals docs nickname it that; that identity is wrong here). Metabase uses the **native Postgres driver** over foreign tables — **no v1 Impala JDBC, no v1 Flight SQL**. DataFusion 50 remains Engine `/sci` + backup. Do **not** COPY product SoR into any Postgres. **AMP packaging is out of scope.** Because Metabase/Postgres may push `SUM` to Impala, **Metabase QP rejection is mandatory**.

HDF5 is treated honestly: `Hdf5FormatModel` is a **pointer-table / metadata-plane façade**, not a JVM Iceberg `FormatModel`. File Format API integration is graduation step 2. `hdf5_iceberg` stays a standalone package (path-dep / submodule / `packages/hdf5_iceberg`), **not** Gaius `thirdparty/`. Polarisfork append lives in Gaius `hx`, wrapping `register_root`.
...
2. **HDF5 is not Iceberg.** Pretending otherwise (registering `.h5` as data files in a Parquet-only table) would break every JVM reader (Impala, Spark, Iceberg 1.10 Java). The pointer-table pattern in design #42 is the honest path.
...
3. **Worked path:** SysML v2 / SDG-grounded HDF5 → `hdf5_iceberg.register_root` → Gaius `hx` Polarisfork pointer tables → Dask hyperslab (script / cyberphy) **and** DataFusion SQL on Parquet overviews/strip. Metabase in-line mapping when the SQL face exists.
4. **Second table:** 10 Hz cognition strip as long-grain Iceberg from a **parallel unpack path** (not `sample_column`).
5. **Engine-first:** `Scientific*` / `CatalogStatus` / `Hdf5Register` RPCs on `:50051` (names chosen to spare existing `SemanticSearch`). CLI `/sci …` is thin. v1 Engine **subprocesses** `signals-df`.
6. **Keep `hdf5_iceberg` standalone** (path-dep/submodule/`packages/hdf5_iceberg`); do not rewrite; do not put it in Gaius `thirdparty/`.

### Non-Goals

- Implementing a JVM Iceberg `FormatModel` for HDF5 in this program of work (graduation step 2; tracked, not delivered).
- A DataFusion **HDF5 TableProvider** in v1 (hyperslabs stay Dask). No `iceberg-datafusion` 0.10 pin on DataFusion 50.
...
- Publishing internal KB content to the public site (`gaius.zndx.org` IRIs for measures).
...
| K2 | **HDF5 stays Layer A (read-only). Iceberg pointer tables + Parquet overviews/stats are Layer B.** `Hdf5FormatModel` remains a shared vocabulary for registration + Dask readers, **not** a JVM `FormatModel`. File Format API is graduation step 2. | Iceberg File Format API (finalized, ships in Iceberg **1.11.0**, TCK in progress) is Java `FormatModel` / `FormatModelRegistry`. No HDF5 implementation exists. PyIceberg and DataFusion 50 read Parquet/ORC/Avro. Registering `.h5` as Iceberg data files would be unread by Impala/Spark. Design #42 already chose this split. |
| K3 | **Two query engines, split by access pattern.** Dask + kerchunk + datashader for 2-D hyperslabs. DataFusion 50 for dimensional SQL over **Polarisfork Parquet only** (PyIceberg current-snapshot file list → `ListingTable`). **No HDF5 TableProvider in DF v1. No `iceberg-datafusion` 0.10** (that release is DataFusion **53** + Arrow 58; Signals pin is DF 50). A Signals DF 53 bump is out of Gaius sequence. | Raster wants byte-range GETs. Dimensional SQL wants Parquet pushdown. Reimplementing `virtual_chunk_bytes` in Rust is a second raster engine. |
...
| K5 | **DAS-like HDF5 is matrix grain; 10 Hz strip is long-grain Iceberg.** Overviews are binned facts with a **distinct measure IRI** (K14). Do not explode `Values(N,T)` as primary warehouse rows. | Binding overview `mean` to `sdg:ProdmlDasRawDataSet` would treat bin averages as raw counts. |
...
| K8 | **`hdf5_iceberg` stays a standalone package.** Consume via path-dep or submodule from `cldr/cybersec/packages/hdf5_iceberg`, or a `packages/hdf5_iceberg` copy with `scripts/vendor-hdf5-iceberg.sh` + NOTICE. **Not `thirdparty/`** (native C++/Java). Polarisfork Iceberg append belongs in **Gaius `src/gaius/hx/`** wrapping `register_root` + `get_catalog()`. The library continues to write warehouse-URI + `parts.parquet` only. cyberphy must not import Gaius. | Avoid circular deps. See A6. |
| K9 | **Aggregation legality is a DataFusion `AnalyzerRule` in the scientific session only** (`--scientific`; backup `service__table` SQL is unchanged). SoR Iceberg `strip_long` stays long-grain. **Engine SQL face:** one **TableProvider** per `measure_iri` (`gaius.cognition.v_<measure>`), **not** a DataFusion `CREATE VIEW`. `#SL.00000008` only when `TableScan.table_name()` is `gaius.cognition.strip_long`. **Dashboard / Postgres-FDW face:** per-measure **Postgres views over FDW foreign tables** (or Impala views imported as FTs) so Metabase never queries polymorphic `strip_long.value`. Postgres may push `SUM` to Impala — **Metabase QP rejection is mandatory**. Same `semantic.aggregations` SoR; two enforcement points: DF analyzer (Engine) + Metabase QP. | DF 50 analyzers see the resolved `LogicalPlan`. FDW is a different path. |
| K10 | **Polarisfork namespaces stay Signals-owned.** Nested identity: namespace `gaius.cognition` + table `strip_long` (not namespace `gaius` + table `cognition.strip_long`). `ScientificQuery` allow-list is the **first path segment** ∈ {`semantic`, `telemetry`, `gaius`} (sqlglot in Gaius; same rule in `signals-df --scientific`). **Deny** first segment `rase`. Reuse `#HX.00000002.NOPOLARIS`. | `_ensure_namespace` creates nested tuples. Exact-match `gaius` would reject `gaius.cognition.*`. |
...
| K12 | **Enhance `impala_fdw` for Kudu and Iceberg. Three Postgres fronts talk to both storages through the FDW.** Polarisfork remains Iceberg REST catalog only (Impala already talks to it; **the FDW does not**). Impala HS2 `:21050` remains the SQL engine. **Lifts** SPEC G6/N2; Principle 6 fail-closed = reject storage that is **neither** Kudu **nor** Iceberg. G1–G5, G7–G10 **and N1** unchanged (no upper-relation pushdown in v1). Iceberg FTs **always** `impala_sql`. Classify via HS2 `DESCRIBE FORMATTED` (§1.1). Replace IMPORT `kudu_only` with `storage_allow 'kudu,iceberg'`. **No `catalog_uri` on CREATE SERVER** — IMPORT is `SHOW TABLES` on HS2 only. **Three fronts:** (1) **Signals devenv Postgres `:5455`** (PG 16, DBs `signals` / `signals_catalog` / `ranger` / `polaris` — **not** PGlite; AGE if present is orthogonal); (2) **system Postgres `:5432`**, schema `warehouse` in database **`sdg`**, `CREATE USER MAPPING FOR metabase`; build `impala_fdw` against **that** major; (3) **AMP PGlite** (Atelier `scripts/pglite-server.mjs` **`:5440`**; Atelier devenv PG **`:5533`**) — **SQL-contract target only**. Stock PGlite does **not** load PGXS `impala_fdw.so` in v1. AMP packaging **out of scope**. v1 proves FDW on **devenv `:5455` + system `:5432`**. Gaius `:5444` is a peer devenv PG — **not** a warehouse. Metabase = native Postgres driver; **no v1 Impala JDBC / Flight**. Lab Metabase path = Iceberg-only FT `sdg.warehouse.strip_long`. UNION view **only when** Kudu `strip_tier0` exists. v1 dashboard aggs are **Postgres-side** (N1). | User decision + implementable PR-S2/S3. Footnote: Signals `governance-scale-plane.md` nicknames `:5455` “pglite”; that is not identity here. |
| K13 | **10 Hz writer:** `channel` is **string** (Parquet dict encoding). Commit **≥60 s** (or row-group target ≥32 MB). Compaction/rewrite is **Signals** `data-product.tier-upkeep`. Pointer table: **unpartitioned** or identity `skos_concept` only. **Never `DayTransform` on `t_min_ns` longs.** Strip partition is identity `epoch_day` (int). | 1–5 s commits at 3 KB/s produce thousands of tiny files/day. |
| K14 | **Overview `mean` binds to `https://signals.zndx.org/gaius/measure/das-overview-amplitude`**, grain `locus-bin×time-bin`, legal **AVG, MIN, MAX, COUNT**. SUM of `mean` is `#SL.00000002`. SUM of `count` (occupancy) may be bound separately. `sdg:ProdmlDasRawDataSet` stays on the HDF5 pointer `measure_iri` + scale, not on overview bins. | `SUM(mean)` ≠ sum of counts. |
| K15 | **Freeze measure IRI form:** `https://signals.zndx.org/gaius/measure/{name}` in Turtle, Iceberg, and Metabase `settings.scientific`. Not `gaius.zndx.org`. Cognition strip is **not** `skos:inScheme SYSML_STRUCTURE`. | Do not put internal measures on the public host. |
...
| K18 | **`ts` for Metabase** is `CAST(ts_ns / 1e9 AS timestamptz)` on **Postgres** `warehouse.v_*` views (and `ts_sql` on Engine TableProviders). Not a fork UnixNano coercion in v1. | One expression both DF listing and PG views can type. |
...
flowchart LR
  ...
  subgraph layerB["Layer B — Polarisfork warehouse"]
    ...
    STRIP["gaius.cognition.strip_long"]
    EVID["rase.evidence"]
  end

  subgraph engines["Query engines"]
    DASK["Dask + kerchunk (cyberphy prod;<br/>Gaius v1 = script + extras)"]
    DF["DataFusion 50<br/>ListingTable over Polarisfork Parquet"]
  end
...
v1 Engine does **not** call Dask. Raster is cyberphy Navigator or `scripts/das_viewport.py` (PR-G7). Metabase cards are dashed until K12 (`sdg.warehouse.*` FTs on `:5432`). Iceberg FTs **always** use `impala_sql`. Do not COPY SoR into devenv Postgres. AMP PGlite is not on this v1 diagram.

**Ownership (non-negotiable):**

| Owns | Who |
|------|-----|
| Polarisfork, RustFS, Impala HS2, Kudu, Kudu→Iceberg settle, `details`/`tx` | **Signals** |
| `impala_fdw` (Kudu **and** Iceberg after G6/N2 lift) | **Signals** — v1 install on devenv `:5455` and system `:5432`. Windows, not SoR. PGlite AMP later. |
| Product identity, HDF5 bytes under `s3://signals-dataproducts/gaius/…`, semantic catalog rows, RASE evidence | **Gaius** (peer) |
| Analog generator + Navigator Dask path | **cyberphy** (until Gaius vendors the generator for the SysML analog) |
| `hdf5_iceberg` library | Origin `cldr/cybersec`; Gaius path-dep / `packages/` copy; Polarisfork writer in Gaius `hx` |
...
- **v1 Metabase path:** IMPORT Iceberg-only `gaius.cognition.strip_long` as `sdg.warehouse.strip_long`. Postgres views `sdg.warehouse.v_ricci` etc. over that FT. **Do not IMPORT** an Impala UNION that references missing Kudu `strip_tier0`.
- **When settle exists:** Impala `CREATE VIEW gaius.cognition.strip AS SELECT … FROM strip_tier0 UNION ALL SELECT … FROM strip_long`, then IMPORT that view as `sdg.warehouse.strip` and retarget `v_*` to it.
...
Origin `audit_local_hdf5` only finds `Values` under root or `ResourceMetrics`. The SysML tree’s `Values` is `/AcquisitionSystem/DasRaw[0]/Values` — **stock audit yields empty geometry**. The layout probe lives in Gaius `src/gaius/hx/sdg_sysml_adapter.py` (wrap `DatasetProvider.audit()`). Do **not** claim origin `audit.py` finds SysML Values. Fail-fast `#SL.00000005.HDF5AUDIT` if neither SysML nor OTel tree is present.
...
|-----------|-------|------|
| SysML layout adapter | **Gaius** `src/gaius/hx/sdg_sysml_adapter.py` (not a vendored file) | Discover `**/*.h5`; tags `product=gaius`, `scheme=sdg`. Origin `audit.py` stays OTel-capable; Gaius probe wraps it. |
| SysML audit branch | same Gaius wrapper | Read `/AcquisitionSystem/...`; extract IRIs, units, uuids. Layout probe mandatory. `semantic_hash` over canonical IRIs. Vendor-sync must not clobber this wrapper. |
| Polarisfork Iceberg append | **Gaius** `src/gaius/hx/semantic_tables.py` `append_pointer_rows` | Extra Iceberg columns (`semantic_hash`, IRIs, `skos_scheme`) are written **here**, not by vendored `DatasetDescriptor`. Library does **not** import Gaius. Fail `#HX.00000002.NOPOLARIS`. |
| Semantic stub IRIs | optional quilt `packages/hdf5_iceberg.patches/` on `dcat_ttl.py` **or** Gaius emit after register | Do not rely on vendor-sync to keep SDG prefixes; quilt separately. |

CLI `hdf5-iceberg register` still writes `parts.parquet` under `--meta`. Engine `Hdf5Register` **must wrap** `DatasetProvider` so `audit()` is the Gaius SysML probe, pass a `LayoutAdapter` **instance** (origin `ADAPTERS` has no `"sdg_sysml"` string), then `register_root`, then `append_pointer_rows`. `skip_audit` is **not** a descriptor hook — do not use it to paper over stock audit.
...
Shape from design #42: virtual-chunk min/max/mean/count; overview bins `dataset_uuid, series_bin, t_bin, vmin, vmax, mean, count`. **`mean` is not raw DAS counts.** Bind `mean` to `https://signals.zndx.org/gaius/measure/das-overview-amplitude` (K14). Bind `count` separately if occupancy SUM is required. These Parquet tables are the DAS facts DataFusion may scan in v1 — not the HDF5 slab.

#### 4.3 Semantic catalog (SoR)

```text
semantic.measures
  measure_iri          string  PK   https://signals.zndx.org/gaius/measure/ricci
                                 or https://signals.zndx.org/sdg#ProdmlDasRawDataSet
  label                string
...
Seed measures (IRI prefix `https://signals.zndx.org/gaius/measure/` unless noted). Legal functions are **`semantic.aggregations` rows**, not a list column:

| measure_iri (suffix) | kind | unit | grain | legal functions |
|----------------------|------|------|-------|-----------------|
| `gpu-power` | extensive | W | device×tick | SUM, AVG, MIN, MAX, COUNT |
| `gpu-util` | intensive | % | device×tick | AVG, MIN, MAX, COUNT |
| `kv-cache` | intensive | 1 | replica×tick | AVG, MIN, MAX, COUNT |
| `run-n` | intensive (occupancy gauge) | 1 | replica×tick | AVG, MIN, MAX, COUNT (**not SUM**) |
| `gen-tps` | rate | 1/s | replica×tick | AVG, MIN, MAX, COUNT |
| `prefill` | rate | 1/s | replica×tick | AVG, MIN, MAX, COUNT |
| `clt` | dimensionless | 1 | process×tick | AVG, MIN, MAX, COUNT |
| `sae` | dimensionless | 1 | process×tick | AVG, MIN, MAX, COUNT |
| `ricci` | intensive | 1 | graph×snapshot | AVG, MIN, MAX, COUNT |
| `ricci-d` | intensive | 1 | graph×snapshot | AVG, MIN, MAX, COUNT |
| `das-overview-amplitude` | intensive (bin mean) | 1 | locus-bin×time-bin | AVG, MIN, MAX, COUNT (**not SUM**) |
| `das-overview-occupancy` | count | 1 | locus-bin×time-bin | SUM, AVG, MIN, MAX, COUNT (binds `overviews_l1.count`) |
...
SQL-facing **TableProviders** (scientific session registers each as its own scan; SoR remains Iceberg `gaius.cognition.strip_long`). **Not** `CREATE VIEW … AS SELECT * FROM strip_long` (that expands to a `strip_long` scan and would fire `#SL.00000008` on legal `AVG`). **Not** one materialized Iceberg table per measure (would fight K5 long-grain SoR).

| TableProvider name | Filter on base `strip_long` | Bound measure |
|--------------------|-----------------------------|---------------|
| `gaius.cognition.v_gpu_power` | `channel IN ('gpu-0',…,'gpu-5')` | `gpu-power` |
| `gaius.cognition.v_gpu_util` | `channel IN ('util-0',…)` | `gpu-util` |
| `gaius.cognition.v_run` | `channel = 'run'` | `run-n` (occupancy; not SUM) |
| `gaius.cognition.v_ricci` | `channel = 'ricci'` | `ricci` |
| … one provider per measure_iri | | |

`semantic.bindings.table_id` is the **provider name** (`gaius.cognition.v_ricci`), not `strip_long`.

`gpu-0..5` are channel **values** (string), not measure IRIs. Dimension `https://signals.zndx.org/gaius/dim/gpu-index` is a column on the power/util views.

Turtle: `src/gaius/semantic/gaius-measures.ttl` with `rdfs:seeAlso` DCGM/vLLM names. **Not** `skos:inScheme SYSML_STRUCTURE`. **Do not** stuff into `gaius_domain.owl`. Promote into SDG corpora when Aegir admits them (`sdg_aperture.py` loads strategy, does not invent).

#### 4.4 10 Hz strip table `gaius.cognition.strip_long`

Long grain, Signals-`details`-adjacent:

| Column | Type | Binding |
|--------|------|---------|
| `ts_ns` | long, required | dimension time |
| `epoch_day` | int, partition | `gaius:dim/epoch_day` |
| `channel` | **string** (Parquet dictionary encoding) | `https://signals.zndx.org/gaius/dim/channel` — SQL uses `channel = 'ricci'` |
| `value` | float | **polymorphic** on this table; scientific SQL uses per-measure **TableProviders** |
| `present` | bool | degenerate (absent = flatline; still a fact) |
| `gpu_index` | int, optional | `gaius/dim/gpu-index` when channel in gpu-*/util-* |
...
```
strip_iceberg_writer  ──reads──►  DCGM :9400  DCGM_FI_DEV_POWER_USAGE (W)
...
Commit **≥60 s** (or buffer until row group ≥32 MB). Tiny-file compaction is Signals `data-product.tier-upkeep`. CI: a busy GPU fixture asserts warehouse `value` for `gpu-0` **> 1.0** (watts, not packed occupancy).

Kudu `details_tier0` settle remains a Signals product path (Q5). Lab v1 writes Iceberg directly. Product id `gaius.cognition.strip`.
...
1. **PyIceberg snapshot resolver (Gaius Python) + DataFusion `ListingTable` (Rust).** Gaius `get_catalog()` loads current snapshots, **refuses delete files** (`#SL.00000012.DFCHILD` if any equality/position delete appears — v1 writers are **append-only**), and writes `--listing` JSON. `signals-df --scientific` has **no** REST Iceberg client. Partition prune is best-effort (identity `epoch_day` on strip).
2. **No HDF5 TableProvider.** Hyperslabs stay Dask. DF never calls `Hdf5FormatModel`.
3. **Analyzer `ScientificAggregationCheck`** enabled **only** with `--scientific`. It keys off **`TableScan.table_name()`**, not the user's original FROM after view expansion:
   - Scan name is `gaius.cognition.strip_long` or `gaius__cognition__strip_long` → any `SUM/AVG/MIN/MAX(value)` → **`#SL.00000008`** (always).
...
Each `v_*` is registered as a **filtered `ListingTable` (or thin wrapper)** sharing the **same Parquet files** as `strip_long`, applying `filter` from the listing, exposing `ts` as the `ts_sql` expression, and advertising `table_name() = gaius.cognition.v_ricci`. DataFusion `CREATE VIEW` is **forbidden** for these names.
...
```json
{
  ...
  "tables": [
    {
      "name": "gaius.cognition.strip_long",
      "namespace": ["gaius", "cognition"],
      "table": "strip_long",
      "files": [
        "s3://signals-dataproducts/iceberg/gaius/cognition/strip_long/data/epoch_day=20260823/part-000.parquet"
      ],
      "schema": {
        "fields": [
  ...
  "views": [
    {
      ...
      "base": "gaius.cognition.strip_long",
      "filter": {"column": "channel", "op": "eq", "value": "ricci"},
      "measure_iri": "https://signals.zndx.org/gaius/measure/ricci",
    ...
    },
    {
      ...
      "measure_iri": "https://signals.zndx.org/gaius/measure/gpu-power",
      "ts_sql": "CAST(ts_ns / 1000000000 AS TIMESTAMP) AS ts"
    }
  ],
  "aggregations": [
    {"measure_iri": "https://signals.zndx.org/gaius/measure/ricci", "function": "SUM", "legal": false, "reason": "intensive"},
    {"measure_iri": "https://signals.zndx.org/gaius/measure/ricci", "function": "AVG", "legal": true, "reason": ""},
    {"measure_iri": "https://signals.zndx.org/gaius/measure/run-n", "function": "SUM", "legal": false, "reason": "occupancy gauge"}
  ]
}
```
...
```json
{"legal": false, "guru": "#SL.00000002.ILLEGALAGG", "reason": "SUM not legal for https://signals.zndx.org/gaius/measure/ricci (intensive)", "measures": ["https://signals.zndx.org/gaius/measure/ricci"]}
```
...
```sql
SELECT SUM(value) FROM gaius.cognition.strip_long WHERE channel = 'ricci';
SELECT SUM(value) FROM gaius.cognition.strip_long;
```
...
SUM is not legal for https://signals.zndx.org/gaius/measure/ricci
...
**PR-G7** is `scripts/das_viewport.py` plus extras `dask`, `datashader`, `h5py`, `kerchunk` (Gaius `pyproject.toml` has `holoviews` only today). Tests: both `--contiguous` and chunked on a **small** fixture; a **documented manual gate** on the 5001×10000 analog for design #42 gate 2 (byte-identical hyperslabs). Colorbar unit from pointer `value_unit`.
...
3. `sdg.warehouse.v_*` Postgres views exist over Iceberg-only FTs (lab) or UNION FT (when `strip_tier0` exists).
...
IMPORT FOREIGN SCHEMA gaius FROM SERVER signals_impala INTO warehouse
  OPTIONS (storage_allow 'iceberg');   -- v1 lab: Iceberg strip_long only
CREATE VIEW warehouse.v_ricci AS
  SELECT CAST(ts_ns / 1000000000 AS timestamptz) AS ts, epoch_day, value, present
  FROM warehouse.strip_long WHERE channel = 'ricci';
...
| `value` | per-measure model only (`v_ricci` → IntensiveQuantity, `v_gpu_power` → ExtensiveQuantity) | Never one model over polymorphic `strip_long.value` |
...
4. `/sci query` legal AVG on overviews; `/sci explain SELECT SUM(value) FROM gaius.cognition.v_ricci` → `#SL.00000002`; `/sci explain SELECT SUM(value) FROM gaius.cognition.strip_long` → `#SL.00000008`.
5. `/metabase project` writes YAML. Cards only after `warehouse.v_*` exist on `:5432`. Overview `mean` is `das-overview-amplitude`, not `ProdmlDasRawDataSet`.

10 Hz companion: unpack writer, **≥60 s** commits, string `channel`, TableProviders `v_ricci` / `v_gpu_power`. CI watts ≫ 1. `run-n` is occupancy (no SUM).
...
from gaius.hx.sdg_sysml_adapter import SysmlDatasetProvider, SysmlLayoutAdapter
from gaius.hx.semantic_tables import append_pointer_rows  # extra Iceberg columns live here

data = SysmlDatasetProvider(  # wraps DatasetProvider.audit() — origin audit.py never sees SysML Values
    roots=["s3://signals-dataproducts/gaius/datasets/hdf5/das/"],
    adapter=SysmlLayoutAdapter(),  # instance, not string "sdg_sysml" (not in origin ADAPTERS)
    readonly=True,
)
...
`Hdf5FormatModel` stays a façade:

```python
# src/hdf5_iceberg/format/model.py — comment already says:
# "Not a JVM Iceberg FormatModel yet — mirrors the File Format API
#  responsibilities so registration + Dask readers share one vocabulary."
```
...
New tables: `telemetry.hdf5_datasets` (+ `semantic_hash`, `skos_scheme`), `telemetry.hdf5_chunk_stats`, `telemetry.overviews_l1`, `telemetry.overviews_l2`, `semantic.measures` (no `aggregations_legal` list), `semantic.dimensions`, `semantic.bindings`, `semantic.aggregations`, `gaius.cognition.strip_long` (`channel` string; optional `strip_wide`). Scientific SQL **TableProviders** `gaius.cognition.v_*` (listing `views[]`; not Iceberg views). Overviews `mean` → `das-overview-amplitude`. Iceberg namespace for the strip is **`gaius.cognition`**, table **`strip_long`**.
...
2. `create_table` with PyIceberg schemas (same style as `create_evidence_table`).
3. Seed `semantic.measures` / `aggregations` from committed Turtle + YAML.
4. Register HDF5 analog; bind columns.
5. Start strip writer (empty table is legal; models project only once bindings exist).
6. No backfill of packed waterfall ring — ring is 60 s RAM, not a warehouse.

Rollback: drop Gaius-created tables (Signals does not `DELETE` product facts; these are Gaius peer tables — dropping is a Gaius operation, not `data-product.tier-upkeep`). Pointer table drop does **not** touch Layer A HDF5.

---

## Alternatives Considered

### A1. Iceberg File Format API for HDF5 vs pointer-table + Parquet overviews

| | File Format API (HDF5 `FormatModel`) | Pointer-table + Parquet overviews (**chosen**) |
|--|--------------------------------------|-----------------------------------------------|
| Status | API finalized; ships Iceberg **1.11.0**; TCK **in progress**; intended examples Vortex/Lance/Comet; **no HDF5 FormatModel** | Running code: `hdf5_iceberg` + design #42 |
| Readers | JVM engines that pick up 1.11 + a hypothetical HDF5 model | Any Parquet Iceberg reader (PyIceberg 0.10, Impala). v1 DataFusion uses `ListingTable` on snapshot files, not iceberg-rust 0.10 |
| HDF5 physics | Could expose row groups as hyperslabs **if** someone writes readers, predicates, deletes | Honest: Iceberg never opens `.h5`; Dask does |
| Catalog swap | Table data files would be `*.h5` | `register_table` of Parquet tables into Polarisfork; HDF5 URIs stay in a string column |
| Risk | High: Java-only API, PyIceberg/DataFusion will lag, Gaius Python engine cannot wait | Low: already the cyberphy law set |

**Decision:** pointer-table now. Graduation step 2: when a real HDF5 `FormatModel` exists **and** PyIceberg/DataFusion can read it, `hdf5_datasets.uri` rows become Iceberg data-file entries; the pointer table remains provenance (design #42 §10). `Hdf5FormatModel` in Python is named to make that graduation a rename-and-wire, not a rewrite.

**Rejected:** registering HDF5 paths in a Parquet table's manifest as if they were Parquet. That would crash Impala.
...
- 10 Hz **SoR = long** (`strip_long`), optional wide view for paint.
...
**Decision:** SDG. `gaius_domain.owl` remains the ThetaAgent subsumption ontology. Cognition-strip measures live in `gaius-measures.ttl` with `rdfs:seeAlso` DCGM/vLLM under `https://signals.zndx.org/gaius/measure/…`, **not** as fake DAS classes, not as `gaius_domain` classes, and **not** as `skos:inScheme SYSML_STRUCTURE`.
...
| **Path-dep + `packages/hdf5_iceberg` copy or submodule** (**chosen**) | Library stays standalone; NOTICE + `scripts/vendor-hdf5-iceberg.sh`; Polarisfork writer in Gaius `hx` | Need a sync script so the copy does not drift |

**Decision:** K8. Polarisfork `append` is Gaius `semantic_tables.py`, not `hdf5_iceberg.metadata.MetadataProvider`.
...
| Public measure IRIs on gaius.zndx.org | Frozen form `https://signals.zndx.org/gaius/measure/…` |
...
| Strip writer | rows/s, file count; **do not** 1–5 s commit. Alert if files ≪ 32 MB **and** commit interval < 60 s; compaction is Signals |
...
| `#SL.00000007.NOLAYOUT` | Unknown `LayoutAdapter` instance **or** `DatasetProvider` that is not the Gaius SysML wrapper | Pass `SysmlLayoutAdapter()`; do not use origin string `sdg_sysml` |
| `#SL.00000008.GRAINMISMATCH` | `TableScan.table_name()` is `gaius.cognition.strip_long`; DAS SUM GROUP BY not XOR | Use `gaius.cognition.v_ricci` TableProvider; XOR one DAS axis on raw counts |
...
| `#SL.00000012.DFCHILD` | `signals-df` missing/crash, or listing snapshot has Iceberg delete files | Set `GAIUS_SIGNALS_DF`; devenv pin PR-S1; v1 is append-only |
...
**CI gate name:** `just semantic-ci` (Gaius) and `just hdf5-iceberg-ci` (package tests). Not smoke. Optional first-run script `scripts/semantic_first_register.sh` may exist as a one-off; do not add `just semantic-smoke`.
...
**Gaius slice (no `:8181` required for unit tests):** package path-dep → SysML analog/audit → `hx` schemas (mocked catalog) → unpack writer tests → `/sci` RPCs with fixture listing → `das_viewport.py` extras → YAML projector → `just semantic-ci`.

**Signals slice:** PR-S1 `signals-df --scientific` analyzer; PR-S2/S3 FDW Iceberg + `:5432/sdg` `warehouse` FTs. **Not** `serve --flight` before Metabase.
...
**Metabase Iceberg cards:** only after FDW Iceberg FTs on `:5432`. Until then YAML-only (not `:3100` as SoR).
...
2. Iceberg File Format API HDF5 `FormatModel` when it exists (Java 1.11+; PyIceberg follow).
3. Puffin: move `hdf5_chunk_stats` into snapshot blobs; keep the table as the human view.
...
|------|----------|------------|
| Iceberg 1.11 File Format API creates pressure to "just write HDF5 FormatModel" | M | Document K2; do not start Java work in Gaius. Track upstream. |
...
| Polymorphic `value` | H | Always `#SL.00000008` on `strip_long`; per-measure views |
...
| Overview SUM of means | H | Distinct measure; SUM illegal |
| `hdf5_iceberg` drift | M | vendor-sync script; Polarisfork writer stays in Gaius hx |
| FDW Iceberg path or S3A→RustFS down | H | YAML only; do **not** use `:3100`/`meta.*` as Iceberg SoR, Flight, or Impala JDBC. `#HX.00000002.NOPOLARIS` |
...
- SQL face → **K12** lift G6/N2; devenv `:5455` + system `:5432`; PGlite AMP is SQL-contract only.
- Dask Engine RPC → **not v1**.

**Still open (do not block Gaius Engine / FDW v1):**

1. **Q4 — Promote measure IRIs into SDG corpora.**
2. **Q5 — Strip settle:** Kudu `*_tier0` then Iceberg `*_tier1`. **v1 Metabase = Iceberg-only** `sdg.warehouse.strip_long`. Impala UNION `strip` **only when** `strip_tier0` exists. Engine DF listing is Iceberg snapshot files.
3. **Q6 — Virtual block size** (design #42).
4. **Q-AMP (out of scope):** How AMP/CAI gets a Postgres that can load PGXS `impala_fdw.so` (sidecar real PG vs extension gap). Do **not** block v1. Same `CREATE EXTENSION` / `CREATE SERVER` / `IMPORT` SQL as devenv.
...
- Impala warehouse: `/home/rch/local/src/wxs/signals/src/signals/ops/warehouse.py` (`ImpalaWarehouse` — Live Iceberg via Impala HS2)
...
Three merge queues. Artifact of Signals PRs is a **versioned `signals-df` binary** (Engine) **plus** `impala_fdw` `.so` and `sdg.warehouse` DDL. Projector syncs the **existing** Metabase Postgres Database (`:5432/sdg`) — **no driver JAR**. Gaius PRs G1–G4 unit-test without `:8181`.
...
- **Depends on:** none
- **Changes:** Origin copy as-is. `uv run hdf5-iceberg --help`. `just hdf5-iceberg-ci` → existing tests. **No Polarisfork writer in the library.** `vendor-hdf5-iceberg.sh` rsyncs origin and **must not clobber** Gaius overlays: exclude `adapters/sdg_sysml.py` and `packages/hdf5_iceberg.patches/*`. Prefer landing SysML probe **in Gaius** (`src/gaius/hx/sdg_sysml_adapter.py` wrapping origin `audit.py`) so a vendor refresh cannot wipe it.

#### PR-G2 — SysML/SDG analog + audit probe

- **Title:** `feat(hdf5): SDG/SysML DAS analog tree and audit probe`
- **Files:** `src/gaius/hx/sdg_sysml_adapter.py` (`SysmlDatasetProvider` wrapping `audit()` + `SysmlLayoutAdapter` instance; **does not edit** vendored `audit.py`); `scripts/generate_sdg_hdf5.py`; extra Iceberg columns only in `append_pointer_rows`; tests 7/5/56 + dtype histogram; **do not** compare SHA to OTel
- **Depends on:** PR-G1
- **Changes:** Layout probe SysML else OTel else `#SL.00000005`. `#SL.00000004` on missing `rdfs.seeAlso`. `skos.broader` not `inScheme`→Concept. Vendor-sync after this PR still cannot clobber the adapter.

#### PR-G3 — Polarisfork schemas + seed (hx wrap)

- **Title:** `feat(hx): telemetry pointer tables and semantic catalog schemas`
- **Files:** `src/gaius/hx/semantic_tables.py`, `src/gaius/semantic/gaius-measures.ttl`, `src/gaius/semantic/seed_aggregations.yaml` (`semantic.aggregations` rows, **no list column**)
- **Depends on:** PR-G2 conceptually
- **Changes:** PyIceberg create via `get_catalog()`. Seed including `das-overview-amplitude`. Tests mock REST. Live `:8181` is integration-only.
...
- **Changes:** JSON guru contract. Listing `gaius.scientific.listing.v1`. Allow-list **first path segment**. `$GAIUS_SIGNALS_DF` then devenv PATH. Limit required. **No Flight, no Impala in this PR** (Engine DataFusion only).

#### PR-G6 — 10 Hz unpack writer

- **Title:** `feat(cognition): Iceberg long-grain writer from DCGM/vLLM/Ricci raw`
- **Files:** `src/gaius/engine/services/strip_iceberg_writer.py` (**not** `sample_column`), bindings, tests
- **Depends on:** PR-G3
- **Changes:** Watts / percent / raw rates; commit ≥60 s; `channel` string; CI watts ≫ 1. Product `gaius.cognition.strip`.

#### PR-G7 — Dask viewport **script** + extras

- **Title:** `feat(semantic): das_viewport.py hyperslab + holoviews unit overlay`
- **Files:** `scripts/das_viewport.py`, `pyproject.toml` extras `dask,datashader,h5py,kerchunk`, small-file tests both layouts
- **Depends on:** PR-G2
- **Changes:** Not an Engine RPC. Manual gate note for 5001×10000 analog. Production Dask remains cyberphy.

#### PR-G8 — YAML Metabase projector (cards gated)

- **Title:** `feat(metabase): project semantic catalog to YAML; cards iff warehouse FTs`
- **Files:** `src/gaius/semantic/project_metabase.py`, `src/gaius/semantic/metabase/models/*.yaml`, CLI `/metabase project`
- **Depends on:** PR-G5, PR-G6, **PR-M1** (types + QP). Iceberg **cards** also need **PR-S2/S3** (FDW Iceberg + `:5432` IMPORT)
- **Changes:** Always YAML. Sync existing Postgres Database; `source-table` = `warehouse.v_*` numeric ids. **No `:3100` fallback.** `#SL.00000010.MBNOPEER`.
...
- **Changes:** Register analog, `/sci explain` illegal SQL, YAML diff. Not smoke.
...
- **Changes:** `--scientific --json --listing` with §5.0 schema. Register `views[]` as **distinct TableProviders** (`table_name()` = `gaius.cognition.v_ricci`), not `CREATE VIEW`. `object_store` RustFS path-style. Tests: (1) local fixture parquet `scheme=file`; (2) documented `s3://signals-dataproducts/iceberg/…` against devenv RustFS or mocked `object_store`. `SUM` on `v_ricci` → `#SL.00000002`; `SUM(value)` on scan `strip_long` → `#SL.00000008`; `AVG` on `v_ricci` **plans** (must not collapse to strip_long). Tag crate/`signals-df-scientific-v1` so Gaius can pin PATH / `GAIUS_SIGNALS_DF`. Backup SQL without flag unchanged.
...
#### PR-S3 — Install FDW on three fronts; lab Iceberg-only FTs

- **Title:** `feat(fdw): INSTALL on Signals devenv :5455 and system :5432; sdg.warehouse`
- **Files:** devenv.nix (`:5455` PG 16); system `:5432` package/build against **that** major; DDL in database **`sdg`**
- **Depends on:** PR-S2
- **Changes:** Windows only — **no COPY**. Lab Iceberg-only `strip_long`. PGlite AMP is **follow-on** (same SQL; no `.so` in v1). Blocks PR-G8 **cards**. Aggregations Postgres-side (N1).
...
**Merge order for a full demo:** PR-S1 (Engine DF), PR-S2 (FDW Iceberg), PR-S3 (`:5432` FTs), PR-M1, Gaius G1–G8 with G8 cards last. **Gaius Engine v1 without Metabase cards** is G1–G7 + G8 YAML + G9 + S1.

...
- **JVM** `org.apache.iceberg.hdf5.Hdf5IcebergFormatModel` — Iceberg File Format API (`FormatModel` / `FormatModelRegistry.register`). Decoder is **pure Java** (K2/K21): **default candidate `io.jhdf:jhdf`**. **No JNI** on Impala/Polaris/Spark for HDF5 reads. ETH CISD `~/local/src/oss/jhdf5` is a **lab oracle only** (optional byte-compare); it is **not** the FormatModel I/O stack.
...
- Shipping CISD **jhdf5 JNI** (`libjhdf5.so`) on Impala / Polarisfork / Spark for HDF5 **reads**. Oracle-only (appendix).
...
| K2 | **This program ships a JVM Iceberg `FormatModel` for HDF5** (`hdf5`) via File Format API (`FormatModel` / `FormatModelRegistry.register`). Decoder is **pure Java — no JNI on Impala**. **Default candidate:** jamesmudd **jhdf** (`io.jhdf:jhdf`, Maven Central **0.13.0** as spike start; pin version/SHA of the **winner** after PR-J1). Quality of both named libraries is **unknown until the analog spike** — do not treat either as proven. Java class **`Hdf5IcebergFormatModel`**. Python façade stays **`hdf5_iceberg.format.Hdf5FormatModel`**. **Comet pattern** ([PR #13786](https://github.com/apache/iceberg/pull/13786)): implement against Iceberg 1.11+ (Signals fork `cldr/signals/components/iceberg`), then extract to `packages/iceberg-hdf5` / Signals module (**not** Gaius `thirdparty/`) and register. TCK [#15415](https://github.com/apache/iceberg/issues/15415) in progress; **gap = our tests** on the SysML analog. **Layer A conservation:** prefer register existing `.h5` as Iceberg `DataFile`s (`format=HDF5`) without rewriting bytes. Pointer-table remains the Python/Dask stepping stone. CISD jhdf5 is **oracle-only**. | `FileFormat` enum today has no HDF5 — enum add on Signals Iceberg fork. jhdf: `HdfFile` / `Dataset.getData(offset, shape)` / `SeekableByteChannel` / HTTP range. |
...
| K21 | **Pure Java decoder on Impala classpath. No JNI for v1 HDF5 reads.** PR-J1 spikes **both** named libraries on the **Gaius SysML analog** (depth 4, 7 groups, 5 datasets, 56 attrs, `Values` int16 hyperslab). Record: slice/hyperslab API, attributes, int16, compound `Windows`, S3/`InputFile` ranged GET. **Winner = first library that reads the analog without JNI.** Default try **jhdf** first; if it cannot hyperslab `Values[series,t]`, try **hdf5io**. If **neither** can → `#SL.00000014.HDF5DECODE` and stop — **do not** silently switch to CISD JNI. Pin winner version/SHA in `packages/iceberg-hdf5`. Register `Hdf5IcebergFormatModel` on Impala Iceberg Java classpath (pure-Java jars only). Until registered, DAS SQL stays Parquet overviews. Metabase/FDW never open `.h5`. CISD `libjhdf5.so` / `java.library.path` live only in **appendix (oracle)**. | User: quality unknown; fail-fast spike; no natives in warehouse JVM. |
...
**Decoder (K21):** pure Java. Default spike `io.jhdf:jhdf:0.13.0`. Adapter wraps Iceberg `InputFile` as `SeekableByteChannel` → `new HdfFile(channel, uri)` (jhdf already has this ctor) or `HdfFile(URL)` HTTP range. Hyperslab: `dataset.getData(sliceOffset, sliceDimensions)` for K20 series-blocks. `ByteBuffer` / `getDataBuffer()` for Iceberg metrics.
...
| Maturity | Maven Central; **0.13.0** (2026-07-26); ~**2320** commits; CI, Sonar, DOI 10.5281/zenodo.3996097; Java 17/21/25 | GitHub **3 commits**; modules hdf5iolib (~90% entities read, ~10% write), hdf5ioapi, hdf5ioit. README lists jHDF and CISD as related work. **Early / unknown.** | Mature JNI wrapper; `HDF5Factory`; `libjhdf5.so`; C under `source/c/`. |
...
| Impala | **No natives.** Pin winner SHA after PR-J1. | Same if it wins spike | **Forbidden** on Impala classpath for v1 reads |

**Decision (K21):** spike **both** on the SysML analog; **jhdf first**. Winner = first that reads analog **without JNI**. Neither works → `#SL.00000014.HDF5DECODE`. CISD remains **optional oracle** (appendix), never silent fallback.
...
| `#SL.00000014.HDF5DECODE` | Neither `jhdf` nor `hdf5io` can open the SysML analog without JNI (slice `Values`, 56 attrs, compound Windows, int16) | Fix analog or library gap. **Do not** load CISD `libjhdf5.so` on Impala. Oracle compare is optional CI only |
...
**Java FormatModel slice:** PR-J1 (jhdf vs hdf5io analog spike) → J2 read path → J3 Impala **pure-Java** classpath → J4 extract. **No `libjhdf5.so`.**
...
**Java slice (this rollout, not later):** PR-J1 spike → PR-J2 `Hdf5IcebergFormatModel` read → PR-J3 Impala classpath (pure Java) → PR-J4 own-repo extract.
...
| JNI `libjhdf5.so` leaking onto Impala | H | K21: oracle appendix only; Impala classpath = FormatModel + jhdf/hdf5io JARs |
...
- **File Format API → K2 in this program.** Decoder is **pure Java** (K21): default **jhdf** (`io.jhdf:jhdf:0.13.0` spike); hdf5io if jhdf cannot hyperslab; **neither → `#SL.00000014`**. CISD jhdf5 **oracle only** — **no JNI** on Impala. Pointer-table stays for Python/Dask.
...
- CISD JHDF5 **oracle only**: `/home/rch/local/src/oss/jhdf5` (`overview.html`, `HDF5Factory`, `libs/native/jhdf5/amd64-Linux/libjhdf5.so`) — **not** Impala runtime
...
#### PR-J1 — Pure-Java decoder spike (fail-fast, no JNI fallback)

- **Title:** `test(iceberg-hdf5): spike jhdf vs hdf5io on SysML analog`
- **Files:** `packages/iceberg-hdf5/spike/**`; fixture from `scripts/generate_sdg_hdf5.py`; comparison table checked in
- **Depends on:** analog generator (PR-G2) or a committed fixture
- **Changes:** Open the **Gaius SysML analog** (depth 4 · 7 groups · 5 datasets · 56 attrs · `Values` int16) with **both** libraries. Record: hyperslab/`getData(offset,shape)`, attributes, int16, compound `Windows`, S3/`InputFile` ranged GET. **jhdf first** (`io.jhdf:jhdf:0.13.0`). If jhdf cannot hyperslab `Values[series,t]`, try hdf5io. **Winner = first without JNI.** If **neither** → `#SL.00000014.HDF5DECODE` and **stop**. **Do not** load CISD `libjhdf5.so`. Optional oracle compare is a **separate** test profile, not the spike pass criterion. Pin winner **version/SHA** in the module.
...
- **Changes:** `readBuilder(InputFile)` via winner (`HdfFile(SeekableByteChannel, URI)` for jhdf). K20 `splitOffsets`. Projection + Iceberg metrics. Tests: analog hyperslab byte-identical to Python `h5py` (and optional CISD oracle). **No write of Layer A.** `dataWriteBuilder` may no-op / unsupported in v1 (conservation).
...
**Merge order for a full demo:** PR-H1 (Python library), PR-S1 (Engine DF Iceberg listing), PR-S2 (FDW Iceberg), PR-S3 (Kudu DDL + UNION FTs on `:5432`), PR-M1, Gaius G1–G8 with G8 cards last. **HDF5-as-Iceberg-files:** PR-J1 → J2 → J3 (Impala) → J4. **Gaius Engine v1 without Metabase cards** is H1 + G1–G7 + G8 YAML + G9 + S1 (DF listing will be empty of strip until first settle; overviews/pointer still work). **Metabase v1 requires S3 UNION + G6 Kudu writer.** Impala HDF5 scans need J3; until then overviews Parquet.

---

## Appendix — CISD JHDF5 oracle (not Impala runtime)

ETH CISD JHDF5 at `/home/rch/local/src/oss/jhdf5` (`ch.systemsx.cisd.hdf5.HDF5Factory`, Apache-2.0 `dist/COPYING`) is **JNI**: `hdf.hdf5lib.H5.loadH5Lib()` → `NativeLibraryUtilities.loadNativeLibrary("jhdf5")` → `libs/native/jhdf5/amd64-Linux/libjhdf5.so` plus C under `source/c/` (HDF5 1.10.3). `HDF5Factory.openForReading` takes `java.io.File`. `BlockwiseMatrixExample` (`blockIndex` / `getMDArrayNaturalBlocks` / `slice`) is a useful **native oracle** for Values hyperslabs.

**Allowed:** optional CI profile that compares jhdf/hdf5io decoded `Values` slices to CISD on a **lab** JVM with `-Dnative.libpath.jhdf5=` / `java.library.path` pointing at `libjhdf5.so`.

**Forbidden in v1 warehouse:** shipping `libjhdf5.so` next to Impala, Polarisfork, or Spark; using CISD as `FormatModel` I/O; silent JNI fallback when the pure-Java spike fails.

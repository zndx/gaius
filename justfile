...
reboot-hardening-install:
    ...
    # polkit: let the gaius user manage systemd units non-interactively, so
    # `just restart` never needs sudo (the units already run as this user; only the
    # manage-units action was gated behind an interactive prompt). Ubuntu 22.04 /
    # polkit 0.105 reads .pkla under localauthority, not JavaScript rules.d.
    sudo install -d -m 0700 /etc/polkit-1/localauthority/50-local.d
    sudo install -m 0644 "$(pwd)/scripts/systemd/50-gaius-units.pkla" /etc/polkit-1/localauthority/50-local.d/50-gaius-units.pkla
